...
export async function onMessage({ event, api, args }) {
    ...
    if (!args[0]) return api.sendMessage('[create, join, end]', threadID, messageID);
    ...
    if (args[0] == 'join') {
        if (!gameThread.player) return api.sendMessage('Nhóm này hiện chưa có nhóm chơi!', threadID, messageID);
...
export async function onReply({ event, api, reply }) {
    var gameThread = global.rps.get(reply.threadID) || {};
    var { win, lose, draw } = gameThread;
    var { rps_bot, list } = reply;
    ...
    if (gameThread.player.length == gameThread.choose.length) {
        api.sendMessage('Tất cả mọi người đã chọn xong, cùng xem kết quả nào...', reply.threadID);
        var msg_win = ''
        var msg_lose = ''
        var msg_draw = ''
        ...
        setTimeout(() => {
            api.sendMessage('》Bot chọn là ' + rps_bot + '\n\n》Những người chiến thắng:\n' + msg_win + '\n》Những người thua:\n' + msg_lose + '\n》Những người hòa: \n' + msg_draw, reply.threadID);
            global.rps.delete(reply.threadID);
        }, 900);
    } else {
        if (success == true) {
            ...
            api.sendMessage(name + ' Đã chọn xong, còn ' + not_ready + ' người chưa chọn!', reply.threadID);
        }
    }
}
