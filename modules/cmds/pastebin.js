export const config = {
    ...
    role: 2,
    author: "D-Jukie",
    viDesc: "Áp dụng code từ buildtooldev và pastebin",
    enDesc: "Apply code from buildtooldev and pastebin",
...
export async function onMessage({ api, event, args }) {
    ...
    if(!text && name) {
        var data = fs.readFile(
          global.client.dirMain +'/modules/cmds/'+ `${args[0]}.js`,
          "utf-8",
          async (err, data) => {
            if (err) return api.sendMessage(`Lệnh ${args[0]} không tồn tại!.`, threadID, messageID);
            const client = new PasteClient("idaSEEu-efVRPsoTsN2lFwdEubeZE0gP");
    ...
    if (url[0].indexOf('buildtool') !== -1 || url[0].indexOf('tinyurl.com') !== -1) {
        ...
        request(options, function (error, response, body) {
            ...
            load('.language-js').each((index, el) => {
                ...
                fs.writeFile(global.client.dirMain +'/modules/cmds/'+ `${args[0]}.js`, code, "utf-8",
                    function (err) {
    ...
    if (url[0].indexOf('drive.google') !== -1) {
      ...
      const path = resolve(global.client.dirMain +'/modules/cmds/'+ `${args[0]}.js`);
      try {
        await global.utils.downloadFile(`https://drive.google.com/u/0/uc?id=${id}&export=download`, path);
